...
0. **Move the GitHub repo to a client-owned account before going further.** Vercel and Supabase are correctly under DPW's accounts; the code shouldn't be the one piece still tied to your personal GitHub. If DPW doesn't have a GitHub org yet, create one (free) — an "Organization" rather than a personal account, so access isn't tied to one person. Then use GitHub's **Settings → General → Danger Zone → Transfer ownership** on `dpw-test` to move it (this preserves full commit history). Afterward, re-point Vercel's GitHub connection at the repo's new location and update your local `origin` remote. Do this before the backend build starts — much less to re-wire later.
